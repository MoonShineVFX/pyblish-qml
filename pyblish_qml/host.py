...
class QtHost(Host):
    def __init__(self):
        ...
        if type(self.app) == QtCore.QCoreApplication:
            try:
                import shiboken2
                self.app = shiboken2.wrapInstance(shiboken2.getCppPointer(QtWidgets.QApplication.instance())[0], QtWidgets.QApplication)
            except ImportError:
                pass
...
def _install_max(use_threaded_wrapper):
    ...
    class PYBLISH_EVENT(QtCore.QEvent):
        ...
        def __init__(self, fn, *args, **kwargs):
            QtCore.QEvent.__init__(self, PYBLISH_EVENT.EVENT_TYPE)
            self.fn = fn
            self.args = args
            self.kwargs = kwargs
    ...
    class Reciver(QtCore.QObject):
        def event(self, event):
            try:
                # print(event.fn.__name__, event.args, event.kwargs)
                result = event.fn(*event.args, **event.kwargs)
                max_to_qml.put(result)
                return True
            except:
